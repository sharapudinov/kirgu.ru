    ...
    if (RSGoPro_OFFERS[element_id]) {
        ...
        if (!!RSGoPro_OFFERS[element_id].OFFERS[finedOfferID].PRICE_MATRIX) {
            ...
            for (var PRICE_CODE in PRICES) {
                ...
                if ($product.find('.js-prices_pdv_' + PRICE_CODE).length > 0) {
                    $product.find('.js-prices_pdv_' + PRICE_CODE).removeClass('new').html(PRICES[PRICE_CODE].PRINT_DISCOUNT_VALUE);
                    if (parseInt(PRICES[PRICE_CODE].DISCOUNT_DIFF) > 0) {
                        $product.find('.js-prices_pdv_' + PRICE_CODE).addClass('new');
                    }
                }

                if ($product.find('.js-prices_pd_' + PRICE_CODE).length > 0) {
                    if (parseInt(PRICES[PRICE_CODE].DISCOUNT_DIFF) > 0) {
                        $product.find('.js-prices_pd_' + PRICE_CODE).html(PRICES[PRICE_CODE].PRINT_DISCOUNT);
                    } else {
                        $product.find('.js-prices_pd_' + PRICE_CODE).html('');
                    }
                }
...
function RSGoPro_OffersExt_PropChanged($optionObj) {
    ...
    if (RSGoPro_OFFERS[element_id] && !$optionObj.hasClass('disabled')) {
        ...
        // set current value
        if ($optionObj.parents('.js-attributes__prop').hasClass('color')) { // color
            $optionObj.parents('.js-attributes__prop').find('.js-attributes__curent-value span span').css({'backgroundImage': $optionObj.find('span').css('backgroundImage')});
            $optionObj.parents('.js-attributes__prop').find('.js-attributes__curent-value span span').attr('title', $optionObj.data('value'));
        } else {
            $optionObj.parents('.js-attributes__prop').find('.js-attributes__curent-value span span').html(value);
        }
        ...
        for (index in RSGoPro_OFFERS[element_id].SORT_PROPS) {
            ...
            arCurrentValues[PROP_CODE] = $optionObj.parents('.js-element').find('.prop_' + PROP_CODE).find('.js-attributes__option.selected').data('value');
            // save next prop_code
        ...
        if (NEXT_PROP_CODE) {
            ...
            $optionObj.parents('.js-element').find('.prop_' + NEXT_PROP_CODE).find('.js-attributes__option').each(function (i) {
                var $option = $(this);
            ...
            // call itself
            var nextOptionObj;
            if (!$optionObj.parents('.js-element').find('.prop_' + NEXT_PROP_CODE).find('.js-attributes__option.selected').hasClass('disabled')) {
                nextOptionObj = $optionObj.parents('.js-element').find('.prop_' + NEXT_PROP_CODE).find('.js-attributes__option.selected');
            } else {
                nextOptionObj = $optionObj.parents('.js-element').find('.prop_' + NEXT_PROP_CODE).find('.js-attributes__option:not(.disabled):first');
            }
            ...
            for (var col in matrix.COLS) {
                ...
                if (!!matrix.MATRIX[col][row]) {
                    ...
                    if ($product.find('.js-prices_pdv_' + matrix.COLS[col].NAME)) {
                        $product.find('.js-prices_pdv_' + matrix.COLS[col].NAME).removeClass('new').html(matrix.MATRIX[col][row].PRINT_DISCOUNT_VALUE);

                        if (parseInt(matrix.MATRIX[col][row].DISCOUNT_DIFF) > 0) {
                            $product.find('.js-prices_pdv_' + matrix.COLS[col].NAME).addClass('new');
                        }
                    }

                    if ($product.find('.js-prices_pd_' + matrix.COLS[col].NAME)) {
                        if (parseInt(matrix.MATRIX[col][row].DISCOUNT_DIFF) > 0) {
                            $product.find('.js-prices_pd_' + matrix.COLS[col].NAME).html(matrix.MATRIX[col][row].PRINT_DISCOUNT_DIFF);
                        } else {
                            $product.find('.js-prices_pd_' + matrix.COLS[col].NAME).html('');
                        }
                    }

                    if ($product.find('.js-prices_pv_' + matrix.COLS[col].NAME)) {
                        if (parseInt(matrix.MATRIX[col][row].DISCOUNT_DIFF) > 0) {
                            $product.find('.js-prices_pv_' + matrix.COLS[col].NAME).html(matrix.MATRIX[col][row].PRIN_VALUE);
                        } else {
                            $product.find('.js-prices_pv_' + matrix.COLS[col].NAME).html('');
                        }
                    }
...
$(document).on('click', '.js-attributes__curent-value', function (e) {
    $('.js-attributes__prop.open:not(.prop_' + $(this).parents('.js-attributes__prop').data('code') + ')').removeClass('open').addClass('close');
    if ($(this).parents('.js-attributes__prop').hasClass('close')) { // was close
        $(this).parents('.js-attributes__prop').removeClass('close').addClass('open');
    } else { // was opened
        $(this).parents('.js-attributes__prop').removeClass('open').addClass('close');
    }
    return false;
});
...
/*$(document).on("click", ".js-attributes__curent-value", function (e) {
    var s = $(this);
    $(".js-attributes__prop.open:not(.js-attributes__code__" + s.parents(".js-attributes__prop").data("code") + ")").removeClass("open").addClass("close");
    if(s.parents(".js-attributes__prop").hasClass("open"))
        s.parents(".js-attributes__prop").removeClass("open").addClass("close")
    else
        s.parents(".js-attributes__prop").removeClass("close").addClass("open"),
    return  false;
}), $(document).on("click", function (e) {
    0 < $(e.target).parents(".js-attributes__prop").length && !$(e.target).parents(".js-attributes__prop").hasClass("js-pic") ;
    $(".js-attributes__prop").removeClass("open").addClass("close")
});*/
