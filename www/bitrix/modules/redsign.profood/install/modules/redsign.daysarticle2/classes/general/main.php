...
class CRSDA2Main
{
	public static function OnBeforePrologElementUpdate() {
		...
		if(CRSDA2Tab::NeedAddTab()) {
			$VALUE_TYPE = 'Perc';
			$VALUE_TYPE = ($_REQUEST['redsign_daysarticle2_discount_type']=='CurEach'?'CurEach':$VALUE_TYPE);
			...
			while($arItems = $dbBasketItems->Fetch()) {
				...
				if($data = $res2->Fetch()) {
					...
					if($arFieldsNew['QUANTITY']<1) {
						$daysarticle2->Delete($data['ID']);
					} else {
						$daysarticle2->Update($data['ID'], $arFieldsNew, true, false);
					}
				} else {
