...
if(COption::GetOptionString("redsign.profood", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "profood_banners_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/_index.php', array('IBLOCK_ID_banners' => $iblockID));
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."include/", array("IBLOCK_ID_banners" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/banners-'.LANGUAGE_ID.'.xml';
$iblockCode = "profood_banners_".WIZARD_SITE_ID;
$iblockType = "presscenter"; 
