...
if(COption::GetOptionString("redsign.profood", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "profood_news_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/_index.php', array('IBLOCK_ID_news' => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/news/index.php', array('IBLOCK_ID_news' => $iblockID));
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."include/", array("IBLOCK_ID_news" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/_all/news-'.LANGUAGE_ID.'.xml';
$iblockCode = "profood_news_".WIZARD_SITE_ID;
$iblockType = "presscenter"; 
