...
if(COption::GetOptionString("redsign.profood", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "profood_action_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/_index.php', array('IBLOCK_ID_action' => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/action/index.php', array('IBLOCK_ID_action' => $iblockID));
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."include/", array("IBLOCK_ID_action" => $iblockID));
		}
	}
	return;
}
