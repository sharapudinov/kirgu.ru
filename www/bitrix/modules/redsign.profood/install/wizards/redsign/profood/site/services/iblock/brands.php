...
if(COption::GetOptionString("redsign.profood", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "profood_brands_".WIZARD_SITE_ID;
		...
		{
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/_index.php', array('IBLOCK_ID_brands' => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH.'/brands/index.php', array('IBLOCK_ID_brands' => $iblockID));
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."include/", array("IBLOCK_ID_brands" => $iblockID));
		}
	}
	return;
}
