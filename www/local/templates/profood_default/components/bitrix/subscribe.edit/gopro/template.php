	...
	if($arResult['ALLOW_ANONYMOUS']=='N' && !$USER->IsAuthorized())
	{
		...
		?><form action="<?=$arResult['FORM_ACTION']?>" method="post"><?
			...
			?><div class="line buttons clearfix"><?
				?><button class="btn-default" type="submit" name="Save" value="<?=( $arResult['ID']>0?GetMessage('CT_BSE_BTN_EDIT_SUBSCRIPTION'):GetMessage('CT_BSE_BTN_ADD_SUBSCRIPTION'))?>" /><?=( $arResult['ID']>0?GetMessage('CT_BSE_BTN_EDIT_SUBSCRIPTION'):GetMessage('CT_BSE_BTN_ADD_SUBSCRIPTION'))?></button><?
			?></div><?
			...
			if ($arResult['ID'] > 0 && $arResult['SUBSCRIPTION']['CONFIRMED'] <> 'Y') {
				?><div class="line border clearfix"><?
					...
					?><button class="btn-primary" type="submit" name="confirm" value="<?=GetMessage('CT_BSE_BTN_CONF')?>" /><?=GetMessage('CT_BSE_BTN_CONF')?></button><?
				?></div><?
			}
		...
		if (!CSubscription::IsAuthorized($arResult['ID'])) {
			?><form action="<?=$arResult['FORM_ACTION']?>" method="post"><?
				...
				?><div class="line clearfix"><?
					...
					?><button class="btn-primary" type="submit" value="<?=GetMessage('CT_BSE_BTN_SEND')?>" /><?=GetMessage('CT_BSE_BTN_SEND')?></button><?
				?></div><?
