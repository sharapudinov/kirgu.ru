...
<div class="sale-profile-detail-link-list">
	<a href="<?=$arParams["PATH_TO_LIST"]?>">&larr; <?=GetMessage("SPPD_RECORDS_LIST")?></a>
</div>
...
if(strlen($arResult["ID"])>0)
{
	...
	<form method="post"  class="sale-profile-detail-form" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
		...
		<div class="sale-personal-profile-detail-form-title-row ">
			<h4><span class="primary-color"><?= Loc::getMessage('SPPD_PROFILE_NO', array("#ID#" => $arResult["ID"]))?></span></h4>
		</div>
		...
		<div class="form-group row">
			...
			<div class="col-md-12">
				<input style="width:100%;" class="form-control" type="text" name="NAME" maxlength="50" id="sale-personal-profile-detail-name" value="<?=$arResult["NAME"]?>" />
			</div>
		</div>
		<?
		foreach($arResult["ORDER_PROPS"] as $block)
		{
			if (!empty($block["PROPS"]))
			{
				?>
				<div class="sale-personal-profile-detail-form-title-row">
					<h4><span class="primary-color"><?= $block["NAME"]?></span></h4>
				</div>
				<?
		...
		<div class="sale-personal-profile-btn-block">
			<input type="submit" class="btn btn-themes btn1 btn-md" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">
			&nbsp;
