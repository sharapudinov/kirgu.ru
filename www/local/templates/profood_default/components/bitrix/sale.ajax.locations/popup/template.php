	...
	&& $arParams['SHOW_QUICK_CHOOSE'] == 'Y')
{
	?><div class="default"><?
		...
		foreach($arParams['LOC_DEFAULT'] as $val)
		{
			...
			?><div><?
				?><input onChange="<?=$arParams['ONCITYCHANGE']?>;" <?=$checked?> type="radio" name="NEW_LOCATION_<?=$arParams['ORDER_PROPS_ID']?>" value="<?=$val['ID']?>" id="loc_<?=$val['ID']?>" /><?
				?><label for="loc_<?=$val['ID']?>"><?=$val['LOC_DEFAULT_NAME']?></label><?
			?></div><?
		}
		?><div><?
			?><input <?if($isChecked!="Y") echo 'checked';?> type="radio" onclick="clearLocInput();" name="NEW_LOCATION_<?=$arParams['ORDER_PROPS_ID']?>" value="0" id="loc_0" /><?
