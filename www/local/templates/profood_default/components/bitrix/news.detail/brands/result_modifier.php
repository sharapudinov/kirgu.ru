...
if ($arResult['PROPERTIES'][$arParams['BRAND_CODE']]['MULTIPLE'] != 'Y') {
    $arResult['FILTER_CONTROL_NAME'] = htmlspecialcharsbx('filter/'.strtolower($arResult['PROP']['CODE']).'-is-'.$arResult['PROPERTIES'][$arParams['BRAND_CODE']]['VALUE']).'/apply/';
} else {
    if (is_array($arResult['PROPERTIES'][$arParams['BRAND_CODE']]['VALUE']) && count($arResult['PROPERTIES'][$arParams['BRAND_CODE']]['VALUE']) > 0) {
        $arResult['FILTER_CONTROL_NAME'] .= 'filter/'.htmlspecialcharsbx(strtolower($arParams['BRAND_CODE']).'-is-'.implode('-or-',$arResult['PROPERTIES'][$arParams['BRAND_CODE']]['VALUE'])).'/apply/';
    }
}
