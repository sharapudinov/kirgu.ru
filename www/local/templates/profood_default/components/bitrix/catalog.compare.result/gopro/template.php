					...
					?><td><?
							...
							?></div><?
								...
								if(is_array($arItem['OFFERS']) && count($arItem['OFFERS'])>0) {
									$crossed_price = '';
									?><div class="price new"><?=GetMessage('PRICE_FROM').' '.$arItem['OFFERS'][0]['MIN_PRICE']['PRINT_DISCOUNT_VALUE'];?></div><?
									...
									$crossed_price = '';
									?><span class="price new"><?=$arItem['MIN_PRICE']['PRINT_DISCOUNT_VALUE']?></span><?
								...
								if($arResult['SHOW_PROPERTIES'][$code]['SHOW']=='Y') {
									?><tr><?
										...
										foreach($arResult['ITEMS'] as $key7 => $arItem) {
											?><td><?
												if (!empty($arItem['DISPLAY_PROPERTIES'][$code]['DISPLAY_PROPERTIES_FORMATED'])) {
													?><span title="<?=strip_tags($arItem['DISPLAY_PROPERTIES'][$code]['DISPLAY_VALUE'])?>"><?=$arItem['DISPLAY_PROPERTIES'][$code]['DISPLAY_PROPERTIES_FORMATED']?></span><?
												} else {
