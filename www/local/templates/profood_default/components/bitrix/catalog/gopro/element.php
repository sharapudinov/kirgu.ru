...
if (\Bitrix\Main\Loader::includeModule('iblock')) {
    ...
    if ($obCache->InitCache(360000, serialize($arFilter) . 'section', '/iblock/catalog')) {
        $arCurSection = $obCache->GetVars();
    } elseif ($obCache->StartDataCache()) {
        $arCurSection = array ();
        $arSelect=array ('ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'UF_*');
        ...
        $sect = $arCurSection;
        if(!($arCurSection['UF_USE_CHEAPER']==1)) {
            $arCurSection['wantCheaperPermitted']=false;

            while (isset($sect['IBLOCK_SECTION_ID'])) {
                $arFilter = array ('ID' => $sect['IBLOCK_SECTION_ID'], 'IBLOCK_ID' => $arParams['IBLOCK_ID']);
                $sect = CIBlockSection::GetList(array (), $arFilter, false, $arSelect)->GetNext();

                $arCurSection['wantCheaperPermitted'] = $arCurSection['wantCheaperPermitted'] || ($sect['UF_USE_CHEAPER']==1);
            }
...
$componentElementParams = array (
    ...
    'USE_CHEAPER'                    => $arCurSection['wantCheaperPermitted']?$arParams['USE_CHEAPER']:'N',
