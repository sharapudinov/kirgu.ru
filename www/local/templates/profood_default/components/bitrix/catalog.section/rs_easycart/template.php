	...
	?><form><?
		...
		if (is_array($arResult['ITEMS']) && count($arResult['ITEMS']) > 0) {
			...
			?><div class="rsec_buttons rsec_clearfix"><?
				...
				if ($ident == 'compare' && ($arParams['COMPARE_RESULT_PATH'] != '' || $arParams['COMPARE_PATH'] != '')) {
					$compareLink = (($arParams['COMPARE_PATH'] != '' && $arParams['DISPLAY_COMPARE'] == 'Y') ? $arParams['COMPARE_PATH'] : $arParams['COMPARE_RESULT_PATH']);
					?><div class="rsec_rightp"><?
						?><input class="btn1 rsec_delsome" type="button" name="Refresh" value="<?=GetMessage('TAB_NAME_GO_COMPARE')?>" onclick="location.href='<?=$compareLink?>';return false;" /><?
					?></div><?
				}
			?></div><?
