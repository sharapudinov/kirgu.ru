...
?><div class="reviewform js-reviewform hide"><?
	?><form action="<?=POST_FORM_ACTION_URI?>#postform" name="<?=$arParams['FORM_ID']?>" id="<?=$arParams['FORM_ID']?>" method="POST" enctype="multipart/form-data" onsubmit="return RSGoPro_SummComment(this);"><?
		...
		?><textarea class="hide" name="REVIEW_TEXT"><?=(isset($arResult['REVIEW_TEXT']) ? $arResult['REVIEW_TEXT'] : '')?></textarea><?
		...
		?><input class="send btn-primary hide" name="send_button" type="submit" value="<?=GetMessage("OPINIONS_SEND")?>" /><?
		?><a class="send btn-primary js-submit" href="#"><?=GetMessage("OPINIONS_SEND")?></a><?
